...
def result():
    ...
    messagebox.showinfo("Game Over!!! ", current_player.name + " wins")
...
def status_bar():
    if current_player == player_1:
        status['text'] = player_2.name + "(" + player_2.color_notation + ")"
    else:
        status['text'] = player_1.name + "(" + player_1.color_notation + ")"
...
def double_click_update(event):
    ...
    if is_obj_movable(a, b, xprev, current_player, ALLOW_SINGLE_CLICK):
        ...
        if not valid_move_has_empty_cell(VALID_MOVES[current_index]):
            ...
            return 0
        canvas.itemconfig(get_oval_obj_key(a, b, current_player), outline="black", width=5)
...
turn = Label(root, text="Turn: ", font="Roboto 12")
turn.pack(side=LEFT, fill=X)
status = Label(root, text=player_1.name +'('+player_1.color_notation+')', font="Times 12 bold", padx=5, pady=5, bd=6, relief="groove")
